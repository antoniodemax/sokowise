...
## Phase 18c — Real-world staging check ☑ (2026-09-18)

Validation only, ahead of giving the staging URL to local business owners. Staging (Vercel + Render + Neon) verified on `main` at head; the full owner journey, tenant isolation, owner/staff permissions, M-Pesa SMS matching, receipt upload, error responses and four viewports checked against the deployed domains with synthetic data (removed afterwards). Two small fixes: the sell screen creates the untracked "Other" product on first use for owners who skipped the starter list, and the insufficient-stock message no longer says "movement". Tester instructions in `docs/REAL_WORLD_TESTING.md`, feedback form in `docs/TESTER_FEEDBACK_TEMPLATE.md`. Still external: Anthropic billing (copilot and receipt reading answer 503 on staging), ephemeral receipt images on Render free, free-tier cold start.
...
`POST /auth/google` (verified Google ID token → session, or a 15-minute signup token for the finish-up form at `/register/google` that collects phone and business), `POST /auth/google/register`; accounts created this way have `password_hash NULL` and `google_sub` set, and password login answers like a wrong password. `POST /auth/password-reset/request` / `confirm`: 6-digit SMS code, hashed, 10 minutes, 5 guesses, single use, every session revoked on success, audited; `/forgot-password` page and a "Forgot password?" link on sign-in. SMS through `app/notifications/sms.py` (Africa's Talking; console sender in development). Migration `728750c20802`. Both features answer "not switched on yet" until `GOOGLE_CLIENT_ID` / `VITE_GOOGLE_CLIENT_ID` and `SMS_PROVIDER` + Africa's Talking credentials are set (OPERATIONS §1). PRD FR-B6/B7, ARCHITECTURE §5.1, DATA_MAPPING §3.2/§3.20.
