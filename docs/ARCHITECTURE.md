...
| Field | Value |
|---|---|
| Status | Draft v0.9 — inventory operations and analytics implemented (§5.10, §5.11) |
...
- **Performance:** one aggregate query per figure (summary runs seven; time series four grouped queries merged by bucket), no per-row round trips, results bounded (`limit` ≤ 200 products). Money returns as NUMERIC → Decimal and is rounded half-up to the cent once, in Python.
