...
## Current state
Phases 1–5 (catalogue half) implemented: `backend/` (FastAPI app factory, settings, JSON logging, error envelope, request-ID middleware, health endpoints, async SQLAlchemy engine/session, the 16 MVP models in `app/models/`, one Alembic migration, PostgreSQL-backed tests, and authentication: Argon2id passwords, JWT access tokens, rotating refresh tokens in an HttpOnly cookie, `/api/v1/auth/*`, `get_current_user` / `get_business_context` / `require_role` in `app/api/deps.py`; business/tenant core: `/api/v1/business`, `/api/v1/users` (OWNER-only member management with last-owner protection), `services/audit.py`, the tenant-isolation test helper in `tests/db/isolation.py`; catalogue: `/api/v1/categories`, `/api/v1/products` with opening-stock `INITIAL` movements through `services/inventory.apply_movement`) and `frontend/` (the Vite scaffold plus a dev proxy for `/api`). No inventory movement, sales, customer, expense or AI endpoints exist yet. Check `docs/ROADMAP.md` for the active phase before starting work, and do not start a later phase without being asked.
