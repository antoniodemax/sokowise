...
- Customer balance = Σ `credit_transactions.amount` (signed: CHARGE +, REPAYMENT −, REVERSAL −, ADJUSTMENT ±); `customers.balance` is only a cache and only `services/credit.post_entry` writes either, under the customer row lock. Repayments beyond the debt need `allow_overpayment`; adjustments never go below zero.
